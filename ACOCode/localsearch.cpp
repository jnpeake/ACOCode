...
*/
    ...
    while ( improvement_flag )
	{
		...
		for (l = 0; l < n; l++)
		{
			...
			pos_c1 = pos[c1];
			s_c1 = tour[pos_c1+1];
			radius = tsp->CalcEdgeDist(c1, s_c1);

			/* First search for c1's nearest neighbours, use successor of c1 */
			for ( h = 0 ; h < nn_ls ; h++ ) 
			{
				c2 = tsp->nnIndexes[c1][h]; /* exchange partner, determine its position */
				if ( c2 != c1 && radius > tsp->CalcEdgeDist(c1, c2) )
				{
					s_c2 = tour[pos[c2]+1];
					gain =  - radius + tsp->CalcEdgeDist(c1,c2) + 
					    tsp->CalcEdgeDist(s_c1,s_c2) - tsp->CalcEdgeDist(c2,s_c2);
					if ( gain < 0 )
					{
			...
			if (pos_c1 > 0)
				...
				p_c1 = tour[n-1];
			radius = tsp->CalcEdgeDist(p_c1,c1);
			for ( h = 0; h < nn_ls ; h++ )
			{
				c2 = tsp->nnIndexes[c1][h];  /* exchange partner, determine its position */
				if ( c2 != c1 && radius > tsp->CalcEdgeDist(c1,c2) )
				{
					...
					if ( p_c1 == c2 )
						continue;
					gain =  - radius + tsp->CalcEdgeDist(c1,c2) + 
						tsp->CalcEdgeDist(p_c1,p_c2) - tsp->CalcEdgeDist(p_c2,c2);
					if ( gain < 0 )
					{
...
void LocalSearch::TwoHalfOpt( int *tour )
{
	int c1, c2;         /* cities considered for an exchange */
    ...
    while ( improvement_flag )
	{
		...
		for (l = 0 ; l < n; l++)
		{
			...
			{
				c2 = tsp->nnIndexes[c1][h]; /* exchange partner, determine its position */
				if ( radius > tsp->CalcEdgeDist(c1,c2) )
				{
					pos_c2 = pos[c2];
					s_c2 = tour[pos_c2+1];
					gain =  - radius + tsp->CalcEdgeDist(c1, c2) + 
					...
					if (pos_c2 > 0)
						...
						p_c2 = tour[n-1];
					gain = - radius + tsp->CalcEdgeDist(c1, c2) + tsp->CalcEdgeDist(c2, s_c1)
						+ tsp->CalcEdgeDist(p_c2, s_c2) - tsp->CalcEdgeDist(c2, s_c2)
						- tsp->CalcEdgeDist(p_c2, c2); 
					if ( c2 == s_c1 )
						gain = 0;
					if ( p_c2 == s_c1 )
						gain = 0;
			...
			for ( h = 0 ; h < nn_ls ; h++ )
			{
				...
				if ( radius > tsp->CalcEdgeDist(c1, c2) )
				{
					...
					s_c2 = tour[pos[c2]+1];
					gain = - radius + tsp->CalcEdgeDist(c2, c1) + tsp->CalcEdgeDist(p_c1, c2)
						+ tsp->CalcEdgeDist(p_c2, s_c2) - tsp->CalcEdgeDist(c2,s_c2)
						- tsp->CalcEdgeDist(p_c2,c2); 
					if ( p_c1 == c2 )
						gain = 0;
...
void LocalSearch::ThreeOpt( int *tour )
{
...
*/
    ...
    pos = (int *)malloc(n * sizeof(int));
	dlb = (int *)malloc(n * sizeof(int));
	h_tour = (int *)malloc(n * sizeof(int));
	hh_tour = (int *)malloc(n * sizeof(int));
	...
	for ( l = 0 ; l < n ; l++ ) {
		...
		c2   = tsp->nnIndexes[c1][h];  /* second city, determine its position */
		pos_c2 = pos[c2];
		s_c2 = tour[pos_c2+1];
		if (pos_c2 > 0)
		    p_c2 = tour[pos_c2-1];
		...
		radius = tsp->CalcEdgeDist(c1, s_c1);
		add1   = tsp->CalcEdgeDist(c1, c2);

		/* Here a fixed radius neighbour search is performed */
		if ( radius > add1 ) {
		    decrease_breaks = - radius - tsp->CalcEdgeDist(c2, s_c2);
		    diffs =  decrease_breaks + add1 + tsp->CalcEdgeDist(s_c1, s_c2);
		    diffp =  - radius - tsp->CalcEdgeDist(c2, p_c2) + 
				tsp->CalcEdgeDist(c1, p_c2) + tsp->CalcEdgeDist(s_c1, c2);
		}
		else 
		    break;
			...
			if ( decrease_breaks + add1 < add2 ) {
				...
				/* We have to add edges (c1,c2), (c3,s_c1), (p_c3,s_c2) to get 
				   valid tour; it's the only possibility */
					...
					gain = decrease_breaks - tsp->CalcEdgeDist(c3,p_c3) +
				    add1 + add2 +
						tsp->CalcEdgeDist(p_c3, s_c2);
				...
				/* We have to add edges (c1,c2), (s_c1,c3), (s_c2,s_c3) */
					...
					gain = decrease_breaks - tsp->CalcEdgeDist(c3, s_c3) +
				    add1 + add2 + 
						tsp->CalcEdgeDist(s_c2, s_c3);
				...
				gain = - radius - tsp->CalcEdgeDist(p_c2, c2) 
				    - tsp->CalcEdgeDist(p_c3, c3) +
				    add1 + add2 + 
				    tsp->CalcEdgeDist(p_c2, p_c3);
				...
				gain = - radius - tsp->CalcEdgeDist(p_c2, c2) - 
				    tsp->CalcEdgeDist(c3, s_c3)
				    + add1 + add2 + tsp->CalcEdgeDist(p_c2, s_c3);
