...
for name in instance_names:
	...
	plot_box( ax, x+0.5, '#339FFF', 1.0, 0.1, stats_no_nn[name] )
	plot_box( ax, x+0.8, 'red', 1.0, 0.1, stats_nn[name] )
	x += 1

# legend
red_patch = mpatches.Patch(color='red', label='VCSS - Xeon Phi')
blue_patch = mpatches.Patch(color='#339FFF', label='vRoulette-1 - Xeon Phi')
green_patch = mpatches.Patch(color='green', label='ACOTSP - CPU')
