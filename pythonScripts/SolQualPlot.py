...
def plot_box( ax, x, boxcol, boxalpha, width, yvals ):
    ...
    ax.plot( box_x, box_y, c='black' )
    ax.plot( [x-width,x+width], [yvals[2],yvals[2]], c='black')
    ax.plot( [x,x], [yvals[1],yvals[0]], c='black')
    ax.plot( [x,x], [yvals[3],yvals[4]], c='black')
    ax.plot( [x-width,x+width],[yvals[0],yvals[0]],c='black')
    ax.plot( [x-width,x+width],[yvals[4],yvals[4]],c='black')
    
def plot_box_white_mean( ax, x, boxcol, boxalpha, width, yvals ):
    box_x = [x-width,x-width,x+width,x+width,x-width]
    box_y = [yvals[1], yvals[3], yvals[3], yvals[1], yvals[1]]
    ax.fill( box_x, box_y, boxcol )
    ax.plot( box_x, box_y, c=boxcol )
    ax.plot( [x-width,x+width], [yvals[2],yvals[2]], c='white')
...
with open('testFinal.csv') as csvfile:
...
for name in instance_names:
	...
	plot_box( ax, x+0.2, 'white', 1.0, 0.1, cpu_stats[name] )
	plot_box( ax, x+0.5, 'gray', 1.0, 0.1, stats_no_nn[name] )
	plot_box_white_mean( ax, x+0.8, 'black', 1.0, 0.1, stats_nn[name] )
	x += 1

# legend
red_patch = mpatches.Patch(color='black', label='VCSS - Xeon Phi')
blue_patch = mpatches.Patch(color='gray', label='vRoulette-1 - Xeon Phi')
green_patch = mpatches.Patch(ec='black',color='white', label='ACOTSP - CPU')
...
plt.savefig("solution_quality.png",dpi=500)
plt.show()
